...
const SND_SRC={
  ...
  // ?v=20260827 = 2026-08-27 에 bgm.mp3 를 모노 48k 로 교체(740KB→369KB). 그전 ?v=20260815 는 1,413KB→740KB.
  // 파일명이 같으면 폰에 남은 옛 파일이 그대로 쓰이므로 날짜표를 붙여 새로 받게 한다.
  bgm:'assets/game/sounds/bgm.mp3?v=20260827',
...
//    첫 프레임은 비교할 직전 값이 없으므로 dt=0 → uf() 안에서 k=1(60fps 취급).
//    상한(100ms)은 uf() 쪽에서 건다 — 여기서 걸면 uf() 를 직접 부르는 곳이 생겼을 때 새어나간다.
// ★ 2026-09-01 ② 루프 중복 시작 가드 — gl() 은 멈추는 장치가 없어(rAF 무조건 재귀) go() 가 두 번 불리면
//    루프가 2개 겹쳐 파리가 정확히 2배가 된다(`script.js:2418` 주석에 이미 적혀 있던 함정).
//    _glOn 으로 '이미 도는 중이면 새로 시작하지 않는다'. 재귀 호출은 인자(ts)가 있으므로 통과시킨다.
var _glPrev=0, _glOn=false, _glFrames=0, _glT0=0, _glDup=0;
function gl(ts){
  if(typeof ts!=='number'){                 // 밖에서 gl() 로 '시작'을 시킨 경우
    if(_glOn){ _glDup++; return; }          // 이미 돌고 있다 → 두 번째 루프를 만들지 않는다(막은 횟수를 센다)
    _glOn=true; _glPrev=0; _glFrames=0; _glT0=0;
  }
  var dt=0;
  if(typeof ts==='number'){
    if(_glPrev) dt=ts-_glPrev;
    _glPrev=ts;
    if(!_glT0) _glT0=ts; else _glFrames++;   // 초당 몇 장 그리는 기기인지 세어 둔다(아래 계측용)
  }
...
function go(mode){
  ...
  // ★ 2026-09-01 ③ 계측 — 이 기기가 1초에 몇 장 그리는지(fps) 를 GA4 로 보낸다.
  //    "고쳐졌는지 무엇으로 아는가" = 실유저 중 90/120Hz 비율이 얼마인지, 그 사람들의 완주율이 오르는지.
  //    델타타임(①)이 들어간 뒤라 fps 가 높아도 파리 속도는 같아야 한다 → 이 값은 '기기 분포'를 보는 용도.
  //    게임 시작 5초 뒤 1회만. 앱이 뒤로 갔다 오면 값이 낮게 잡히므로 5fps 미만은 안 보낸다.
  //    ⭐ dup = '이미 도는데 또 시작하려다 가드에 막힌 횟수'. 이게 있어야 두 경우가 갈린다:
  //      fps≈60 + dup≥1 → 모드 재선택 등으로 go()→gl() 이 두 번 불린 것(예전엔 이게 곧 2배였다)
  //      fps≈120 + dup=0 → 진짜 고주사율 화면(WebView·OS 쪽)
  //      가드가 두 번째 루프를 막으므로 fps 만으로는 두 경우가 구분되지 않는다.
  //    ⚠️ GA4 맞춤 측정기준 등록 필요(사장님 몫): fps · dup
  try{ setTimeout(function(){
    try{
      if(!_glT0||_glFrames<5) return;
      var sec=(performance.now()-_glT0)/1000; if(sec<1) return;
      gtag('event','frame_diag',{game_mode:gameMode, fps:Math.round(_glFrames/sec), dup:_glDup});
    }catch(e){}
  },5000); }catch(e){}
  // 모드별 배경 + 색감 전환
  const gc=document.getElementById('gc');
...
/* ══ 화면 크기가 바뀌면 과일 퍼즐판을 다시 그린다 (2026-08-30) ══════════════
   ...
   열었다가 가로로 돌리면 옛 크기 좌표가 그대로 남아 조각이 판 밖으로 밀린다
...
function goAnimalPuzzle(){
  try{
    ...
    bk.style.cssText='position:fixed;top:14px;left:calc(50vw - min(50vw,280px,31vh) + 14px);z-index:100000';   // 화면 왼쪽이 아니라 '판' 왼쪽 끝
...
/* ── 퍼즐 제목을 판에 맞추기 (2026-08-30) ─────────────────────────────────
   뒤로 화살표를 '판 왼쪽 끝' 으로 옮기면서 제목(화면 전체 기준 가운데)과 겹치게 됐다.
   ⚠️ 폭을 판 기준으로 바꾸는 것만으로는 안 된다 — 844×390 이면 판 242px 안에
      제목 226.7px 와 화살표 66px 가 동시에 못 들어간다(가운데를 지키려면 9px 이하여야 한다).
      그래서 ①옆에 세워보고 ②안 되면 화살표 아래로 내린다.
   ⚠️ 글자수×추정폭 같은 공식을 쓰지 말 것(CLAUDE.md 안정성 규칙 8번). 실측한다.
...
function wpFitTitle(){
  try{
    ...
    el.style.fontSize='';                                        // 못 세우면 화살표 아래로
    if(bk){
      var br2=bk.getBoundingClientRect(), wr=wp.getBoundingClientRect();
      el.style.marginTop=(br2.bottom-wr.top+6)+'px';
    }
