...
function goAnimalPuzzle(){
  try{
    ...
    fr.src='test/animal-fit.html?b='+Date.now();          // HTML 항상 최신(미디어는 내부에서 캐시)
    ...
    bk.style.cssText='position:fixed;top:14px;left:calc(50vw - min(50vw,280px,31vh) + 14px);z-index:100000';   // 화면 왼쪽이 아니라 '판' 왼쪽 끝 (넓은 화면에서 검은 여백에 홀로 뜨지 않게)
...
function goDinoPuzzle(){
  try{
    ...
    fr.setAttribute('allow','autoplay; fullscreen');
    fr.style.cssText='border:0;width:100%;height:100%;display:block;';
    fr.src='test/dino-fit.html?b='+Date.now();            // HTML 항상 최신(미디어는 내부에서 캐시)
    var bk=document.createElement('button');
    bk.setAttribute('aria-label','back');
...
function openPhonicsGame(setNo){
  try{
    ...
    fr.setAttribute('allow','autoplay; fullscreen');
    fr.style.cssText='border:0;width:100%;height:100%;display:block;';
    fr.src='test/phonics/index.html?b='+Date.now();   // HTML 항상 최신(미디어는 내부에서 캐시)
    var bk=document.createElement('button');
    bk.setAttribute('aria-label','back');
