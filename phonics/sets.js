...
window.PHONICS_SETS = {
  ...
  /* ── 세트 2 : m d g o c k ── 준비 중. **아직 잠겨 있다**(script.js 의 PH_SETS 2번 open:false).
       ✅ **6단어 에셋이 다 찼다**(2026-09-06). videos 가 빈 항목은 하나도 없다.
          (그전까지는 에셋이 없는 단어의 videos 를 빈 배열로 뒀었다 — 코드가 없는 영상을
           조용히 건너뛰므로 글자 맞추기·음가·통발음까지는 그대로 돌아갔다.)
       ⚠️ 단어를 열려면 단어마다 **통발음(word_*.mp3) + 보상 영상 3편**이 필요하다.
       ✅ **화질 티어 36개(videos_650/ · videos_400/) 완료**(2026-09-07, 커밋 dfbb514).
          650 합계 10.58MB · 400 합계 6.59MB. 세트1과 같은 방식(-preset slow / 2-pass / -an)이다.
       ✅ **통발음 6종 음량 레벨링 완료**(2026-09-07, 커밋 26e618c). 편차 11.8dB → 2.0dB.
          ⛔ 새 단어를 더할 때는 **티어 2개도 같이** 만들 것. 안 만들면 느린 회선에
             원본(편당 1.5~1.8MB)이 그대로 나간다(CLAUDE.md C-2 항목).
       🔴 **남은 조건은 "폰 확인" 하나뿐이다.** 그것만 끝나면 잠금을 푼다. ── */
  2: {
    id: 'set2',
    letters: ['m','d','g','o','c','k'],
    words: [
      ...
      //   ※ 티어 관련 옛 메모는 위 세트2 머리말로 합쳤다(2026-09-07). 36개 전부 완료됐다.
      ...
      // dig — 동작 단어라 pat·nap 과 같은 짜임: 뜻이 제일 깨끗하게 보이는 조용한 장면 먼저,
      //   가장 친숙한 동물로 활기차게 마무리. 두더지(구멍에서 흙만 밀어냄) → 오소리(풀언덕을 앞발로)
      //   → 강아지(흙 튀기며 파다가 마지막에 구덩이 위로 올라와 웃는 얼굴).
      //   ⚠️ dig_mole 은 배경이 단순한 만화체라 나머지(수채화 그림책풍)와 그림체가 다르다.
      //     2026-09-06 사장님이 **움직이는 걸로 보면 어색하지 않다**고 확인 → 그대로 쓴다.
      //     (sip 의 3D 렌더 3편과 같은 종류의 건이지만 이쪽은 판단이 끝났다)
      {word:'dig', tokens:['d','i','g'], traps:[], videos:['mole','badger','dog']},
      // kid — 순서는 하루의 흐름이다: 밖에서 놀고(bicycle) → 들어와 책 보며 아이스크림(book)
      //   → 이 닦기(toothbrush). 자전거가 첫 편인 것은 움직임이 가장 커서다
      //   (첫 영상에는 음가 강조가 안 붙으므로 눈으로 붙잡아야 한다 — cat 의 ball 과 같은 이유).
      //   ⚠️ 아이스크림은 따로 있는 영상이 아니라 `kid_book` 안에 들어 있다
      //     (소파에서 아이스크림을 들고 그림책을 보는 장면). "아이스크림 영상"을 찾지 말 것.
      {word:'kid', tokens:['k','i','d'], traps:[], videos:['bicycle','book','toothbrush']}
    ]
  }
