...
- [ ] 🟡 **퍼즐 메뉴에 인사 영상 겹침** (2026-07-28 원인 규명 완료, 수정 대기) — `greet()`가 "시작화면 벗어남"을 `#ss`의 display로 판정하는데 `goWordCat()`은 `#ss`를 안 숨김(덮기만) → 메뉴 진입 후 뒤늦게 greet 발동, 영상 z-index 200 > `.wc` 118이라 위에 겹침. 보강 타이머도 같은 조건이라 무력. 수정안=greet 조건을 "`.ms`/`.wc`/`.wp` 안 열림"으로 변경 or 영상 z-index 낮추기. 상세는 CHANGELOG 2026-07-28
