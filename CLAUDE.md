...
- 🐸 **시작화면 인사(손흔들기) 끄기 — 테스트판 검증 완료, 라이브 반영 대기**(2026-08-10, 커밋 `c527517`). 첫 화면엔 앉은 개구리만. 액션 클립은 게임 안에서만 유지. `test/frog-reactions-nogreet.js`의 `GREET_ON=false` 두 줄 + 헤더 주석 정정을 라이브 `frog-reactions.js`에 옮기면 끝. ※ 인사는 2026-07-21 `a8dfd11`로 들어간 **의도된 기능**이었고, "나올 때·안 나올 때"가 갈린 주원인은 **앱 완전종료 후 켜기 vs 홈에서 복귀**(MainActivity `loadUrl`이 `onCreate`에만 있음). 자세한 건 CHANGELOG 2026-08-10
