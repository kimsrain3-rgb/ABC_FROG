...
- 🐛 **"로딩 중 버튼 먹통" 발견 → 부팅 가드 테스트 빌드 완료(폰 확인 대기)** — 시작화면 버튼은 HTML만으로 즉시 눌리는데 처리 함수(인라인 onclick 11개)는 body 끝 `script.js` 도착 후에야 생김 → 느린 네트워크(멕시코·필리핀·네팔)에서 `ReferenceError`. `window.onerror`가 삼켜서 **조용히 무반응**이었음. `test/current-bootguard.html`에서 Playwright 검증 완료(커밋 `099dcad`). **폰 확인 후 라이브 `index.html` 반영 필요.** 자세한 건 CHANGELOG 2026-08-05
...
- 🚧 **작업 중(미완): 시작화면 ABC/Word/Phonics 3버튼** — 배치=세로목록안 확정, **코드 미착수**. 남은 결정 2건(누를 때 동작 / 아이콘 모양)은 CHANGELOG 2026-08-04 "이어서·미완" 참고
