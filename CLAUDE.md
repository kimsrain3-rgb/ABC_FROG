...
- 🗣️ **파닉스 세트 선택 메뉴 라이브 완료**(2026-08-13, 커밋 `37b3019`, 백업태그 `backup-before-phonicsmenu-20260813`) — Phonics 버튼 → **세트 메뉴(카드 4장)** → Phonics 1 = satpin 게임. 2·3·4는 잠금(`wcLocked` 재사용). ⚠️ **메뉴 화면(`#ps`)은 `index.html` 이 아니라 `script.js` 의 `_buildPhonicsMenu()` 가 만든다** — `index.html` 은 캐시버스터가 없어 옛 판이 폰에 남기 때문(2026-08-12 파닉스 잠김과 같은 함정). 새 세트를 열려면 `PH_SETS` 의 `open:true` + `goPhonicsSet()` 분기만 고치면 된다. 자세한 건 CHANGELOG 2026-08-13
...
- 🎬 **C-2 적응형 보상 영상 완료(공룡·동물·파닉스 38편)**(2026-08-17~18, 마지막 커밋 `5ee0c8b`, 백업태그 `backup-before-adaptive-video-20260817`) — 회선에 맞춰 원본/650KB/400KB 를 자동으로 고른다. **2g 기준 76.3→14.2MB(−81%)**. 공용 파일 `video-quality.js` 의 `pick()`·`original()`·`reportFallback()` 을 4개 문서가 함께 쓴다. ⚠️ **주소 조립 순서 = `pick()` 먼저, 캐시 꼬리표(`bust()`/`VID_VER`) 나중.** ⚠️ 새 보상 영상을 추가하면 **티어 2개도 같이 만들어야** 폴백으로 원본이 나가 무거워진다. 비상 스위치 = `FORCE_ORIGINAL=true`. 파닉스는 소리 트랙 없음(`-an`, 어차피 muted). 🔴 **GA4 맞춤 측정기준 5개 등록은 아직 안 됨**(`tier`·`reason`·`effective_type`·`save_data`·`puzzle`) — 등록 전 데이터는 보고서에서 안 쪼개짐. 자세한 건 CHANGELOG 2026-08-17·08-18
...
- [ ] 🔴📊 **파닉스 완성 이벤트 없음** — 2026-08-12 공개 후 **완주율 데이터 0.** `phonics_open`(열었다)만 있고 "끝냈다"가 없다(`postMessage`도 `word_puzzle_complete`도 없음) → **파닉스 2~4단계 개설 판단 불가.** 표준 방식은 위 "측정 없이 내보내지 않는다" 참고
