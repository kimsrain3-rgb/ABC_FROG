...
- [ ] 🔵 **동물 퍼즐 시즌1 (친근한 동물 10종)** — 진행 **9/10** (🦝너구리 #5만 남음). 배경 통째 직소퍼즐 + 이름 스펠링 + 동물 영상 보상. 코드=`test/current.html`(테스트 배포본)+`proto-animal.html`(원본) 의 `ANIMALS` 배열(둘 동기화). **본게임 미통합**(테스트=고정주소 /test/). 순서:
  1.🐘코끼리✅ 2.🦁사자✅ 3.🐻곰✅ 4.🐰토끼✅ **5.🦝너구리⬜(에셋대기, 원래 다람쥐→너구리로 변경)**
  6.🦒기린✅ 7.🐯호랑이✅ 8.🐱고양이✅ 9.🐶강아지✅ 10.🐨코알라✅(피날레)
  - ※ 펭귄·고래·원숭이 제외(2026-06-25 시즌1 재정비). 출시 전 PNG들(곰·호랑이·토끼·고양이·강아지) JPG화. 다음 큰 단계=**본게임 통합**(시작화면 Animal 카드 잠금해제 + script.js 통합 → 실유저 플레이).
