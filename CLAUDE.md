...
- 🍎 **그림 다이어트 1a(사과) 라이브 완료 / 1b(나머지 9장) `test/` 폰 확인 대기**(2026-08-20, 커밋 `c3c7182`·`27d5f58`) — 과일 퍼즐 그림을 **원본 해상도 그대로 두고 WebP 손실 q80·method6** 으로만 줄인다. 10장 **16.4→0.77MB(−95.3%, 14.87MB 절약)**. ⚠️ **PNG 256색은 쓰지 말 것** — 팔레트 변환이 반투명 가장자리를 뭉개 **투명 픽셀 수가 바뀌고**, 퍼즐 조각 마스크는 그림의 **알파(`alpha>20`)에서만** 만들어지므로 조각이 어긋난다. WebP 손실은 알파를 무손실로 넣어 **비트 단위로 안 변한다**(9장 실측 최대차 0). ⚠️ 확장자가 바뀌어 주소가 달라지므로 `?v=` 갱신 불필요. 원본 PNG 는 지우지 않는다(되돌릴 땐 주소만 `.png` 로). 자세한 건 CHANGELOG 2026-08-20
